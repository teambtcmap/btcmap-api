use super::{queries, schema::AreaElement};
use crate::Result;
...
pub async fn insert(area_id: i64, element_id: i64, pool: &Pool) -> Result<AreaElement> {
    pool.get()
        .await?
        .interact(move |conn| queries::insert(area_id, element_id, conn))
        .await?
}
...
pub async fn select_by_area_id(area_id: i64, pool: &Pool) -> Result<Vec<AreaElement>> {
    pool.get()
        .await?
        .interact(move |conn| queries::select_by_area_id(area_id, conn))
        .await?
}
...
pub async fn set_deleted_at(
    ...
    pool.get()
        .await?
        .interact(move |conn| queries::set_deleted_at(id, deleted_at.as_ref(), conn))
        .await?
}
