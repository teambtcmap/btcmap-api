use super::{blocking_queries, schema::ElementEvent};
use crate::Result;
use deadpool_sqlite::Pool;
use serde_json::Value;
use std::collections::HashMap;
use time::OffsetDateTime;

pub async fn insert(
    user_id: i64,
    element_id: i64,
    r#type: impl Into<String>,
    pool: &Pool,
) -> Result<ElementEvent> {
    let r#type = r#type.into();
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::insert(user_id, element_id, &r#type, conn))
        .await?
}

pub async fn select_created_between(
    period_start: OffsetDateTime,
    period_end: OffsetDateTime,
    pool: &Pool,
) -> Result<Vec<ElementEvent>> {
    pool.get()
        .await?
        .interact(move |conn| {
            blocking_queries::select_created_between(&period_start, &period_end, conn)
        })
        .await?
}

pub async fn select_all(
    sort_order: Option<String>,
    limit: Option<i64>,
    pool: &Pool,
) -> Result<Vec<ElementEvent>> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::select_all(sort_order, limit, conn))
        .await?
}

pub async fn select_by_user(id: i64, limit: i64, pool: &Pool) -> Result<Vec<ElementEvent>> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::select_by_user(id, limit, conn))
        .await?
}

pub async fn select_by_type(
    r#type: String,
    sort_order: Option<String>,
    limit: Option<i64>,
    pool: &Pool,
) -> Result<Vec<ElementEvent>> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::select_by_type(&r#type, sort_order, limit, conn))
        .await?
}

pub async fn select_updated_since(
    updated_since: OffsetDateTime,
    limit: Option<i64>,
    pool: &Pool,
) -> Result<Vec<ElementEvent>> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::select_updated_since(updated_since, limit, conn))
        .await?
}

pub async fn select_by_id(id: i64, pool: &Pool) -> Result<ElementEvent> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::select_by_id(id, conn))
        .await?
}

pub async fn patch_tags(
    id: i64,
    tags: HashMap<String, Value>,
    pool: &Pool,
) -> Result<ElementEvent> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::patch_tags(id, &tags, conn))
        .await?
}

#[cfg(test)]
pub async fn set_updated_at(
    id: i64,
    updated_at: time::OffsetDateTime,
    pool: &Pool,
) -> Result<ElementEvent> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::set_updated_at(id, updated_at, conn))
        .await?
}
