use super::{blocking_queries, schema::ElementComment};
use crate::Result;
...
pub async fn insert(
    ...
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::insert(element_id, comment, conn))
        .await?
}
...
pub async fn select_by_element_id(
    ...
    pool.get()
        .await?
        .interact(move |conn| {
            blocking_queries::select_by_element_id(element_id, include_deleted, limit, conn)
        })
        .await?
}
