use super::{blocking_queries, schema::Report};
use crate::Result;
...
pub async fn insert(area_id: i64, date: Date, tags: JsonObject, pool: &Pool) -> Result<Report> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::insert(area_id, date, &tags, conn))
        .await?
}
...
pub async fn select_by_date(date: Date, limit: Option<i64>, pool: &Pool) -> Result<Vec<Report>> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::select_by_date(date, limit, conn))
        .await?
}
...
pub async fn select_latest_by_area_id(area_id: i64, pool: &Pool) -> Result<Report> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::select_latest_by_area_id(area_id, conn))
        .await?
}
...
pub async fn select_by_area_id(
    ...
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::select_by_area_id(area_id, limit, conn))
        .await?
}
