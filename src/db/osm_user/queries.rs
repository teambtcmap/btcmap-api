...
pub async fn insert(id: i64, osm_data: EditingApiUser, pool: &Pool) -> Result<OsmUser> {
    pool.get()
        .await?
        .interact(move |conn| super::blocking_queries::insert(id, &osm_data, conn))
        .await?
}
...
pub async fn set_tag(id: i64, name: String, value: Value, pool: &Pool) -> Result<OsmUser> {
    pool.get()
        .await?
        .interact(move |conn| super::blocking_queries::set_tag(id, &name, &value, conn))
        .await?
}
...
pub async fn select_all(limit: Option<i64>, pool: &Pool) -> Result<Vec<OsmUser>> {
    pool.get()
        .await?
        .interact(move |conn| super::blocking_queries::select_all(limit, conn))
        .await?
}
...
pub async fn select_updated_since(
    ...
    pool.get()
        .await?
        .interact(move |conn| {
            super::blocking_queries::select_updated_since(&updated_since, limit, conn)
        })
        .await?
}
...
pub async fn select_by_id_or_name(id_or_name: String, pool: &Pool) -> Result<OsmUser> {
    pool.get()
        .await?
        .interact(move |conn| super::blocking_queries::select_by_id_or_name(&id_or_name, conn))
        .await?
}
...
pub async fn select_by_id(id: i64, pool: &Pool) -> Result<OsmUser> {
    pool.get()
        .await?
        .interact(move |conn| super::blocking_queries::select_by_id(id, conn))
        .await?
}
...
pub async fn select_by_name(name: String, pool: &Pool) -> Result<OsmUser> {
    pool.get()
        .await?
        .interact(move |conn| super::blocking_queries::select_by_name(&name, conn))
        .await?
}
...
pub async fn set_osm_data(id: i64, osm_data: EditingApiUser, pool: &Pool) -> Result<()> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::set_osm_data(id, &osm_data, conn))
        .await?
}
...
pub async fn remove_tag(id: i64, name: String, pool: &Pool) -> Result<OsmUser> {
    pool.get()
        .await?
        .interact(move |conn| blocking_queries::remove_tag(id, &name, conn))
        .await?
}
