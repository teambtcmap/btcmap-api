...
pub async fn insert(tags: Map<String, Value>, pool: &Pool) -> Result<Area> {
    pool.get()
        .await?
        .interact(|conn| super::queries::insert(tags, conn))
        .await?
}
...
pub async fn select(
    ...
    pool.get()
        .await?
        .interact(move |conn| super::queries::select(updated_since, include_deleted, limit, conn))
        .await?
}
...
pub async fn select_by_search_query(
    ...
    pool.get()
        .await?
        .interact(|conn| super::queries::select_by_search_query(search_query, conn))
        .await?
}
...
pub async fn select_by_id_or_alias(id_or_alias: impl Into<String>, pool: &Pool) -> Result<Area> {
    ...
    pool.get()
        .await?
        .interact(|conn| super::queries::select_by_id_or_alias(id_or_alias, conn))
        .await?
}
...
pub async fn select_by_alias(alias: impl Into<String>, pool: &Pool) -> Result<Area> {
    ...
    pool.get()
        .await?
        .interact(|conn| super::queries::select_by_alias(alias, conn))
        .await?
}
...
pub async fn patch_tags(area_id: i64, tags: Map<String, Value>, pool: &Pool) -> Result<Area> {
    pool.get()
        .await?
        .interact(move |conn| super::queries::patch_tags(area_id, tags, conn))
        .await?
}
...
pub async fn remove_tag(area_id: i64, tag_name: impl Into<String>, pool: &Pool) -> Result<Area> {
    ...
    pool.get()
        .await?
        .interact(move |conn| super::queries::remove_tag(area_id, tag_name, conn))
        .await?
}
...
pub async fn set_updated_at(id: i64, updated_at: OffsetDateTime, pool: &Pool) -> Result<Area> {
    pool.get()
        .await?
        .interact(move |conn| super::queries::set_updated_at(id, &updated_at, conn))
        .await?
}
...
pub async fn set_deleted_at(
    ...
    pool.get()
        .await?
        .interact(move |conn| super::queries::set_deleted_at(id, deleted_at, conn))
        .await?
}
