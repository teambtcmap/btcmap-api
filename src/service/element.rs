            ...
            match old_issue {
                Some(old_issue) => {
                    if old_issue.deleted_at.is_some() {
                        db::main::element_issue::queries::set_deleted_at(old_issue.id, None, pool)
                            .await?;
                    }
                    if old_issue.severity != issue.severity {
                        db::main::element_issue::queries::set_severity(
                            old_issue.id,
                            issue.severity,
                            pool,
                        )
                ...
                None => {
                    db::main::element_issue::queries::insert(
                        element.id,
                        issue.code(),
                        issue.severity,
                        pool,
...
pub fn generate_tags(
    ...
    for tag in &include_tags {
        match *tag {
            ...
            "payment_provider" => {
                if let Some(payment_provider) = element.payment_provider() {
                    res.insert("payment_provider".into(), json! { payment_provider });
                }
            }
            "telegram" => {
                if let Some(it) = element.telegram() {
                    res.insert("telegram".to_string(), it.into());
                }
            }
            unrecognized_tag => {
