...
#[get("/top")]
pub async fn get_top(pool: Data<MainPool>) -> Res<Vec<Community>> {
    let communities = crate::db::main::area::queries::select_top_communities(&pool)
        .await
        .map_err(|_| RestApiError::database())?;

    let result: Vec<Community> = communities
        .into_iter()
        .map(|c: CommunityStats| Community {
            id: c.id,
            alias: c.alias,
            name: c.name,
            icon: c.icon_url,
