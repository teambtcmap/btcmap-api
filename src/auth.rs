...
pub fn is_from_admin(req: &HttpRequest) -> Result<(), ApiError> {
    ...
    if admin_token.len() == 0 {
        return Err(ApiError::new("Admin token is not set"));
    }
    ...
    if auth_header.len() == 0 {
        return Err(ApiError::new("Authorization header is missing"));
    }

    let auth_header_parts: Vec<&str> = auth_header.split(" ").collect();
    if auth_header_parts.len() != 2 {
        return Err(ApiError::new("Authorization header is invalid"));
    }

    if auth_header_parts[1] != admin_token {
        return Err(ApiError::new("Invalid token"));
    }
