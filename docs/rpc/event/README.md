...
Each event has its own row in the `event` table. Every event must have a
`starts_at` timestamp. Deletion is soft (`deleted_at` is set to non-null deletion
timestamp). Use `get_events` with `include_deleted` to surface tombstones, and
